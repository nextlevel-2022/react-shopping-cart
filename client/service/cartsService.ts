...
export const isMaxCartItemQuantity = (currentQuantity: CartItem['quantity']) => {
  const MAX_CART_ITEM_QUANTITY = 20;

  return MAX_CART_ITEM_QUANTITY === currentQuantity;
};

export const isMinCartItemQuantity = (currentQuantity: CartItem['quantity']) => {
  const MIN_CART_ITEM_QUANTITY = 1;

  return MIN_CART_ITEM_QUANTITY === currentQuantity;
};

export const hasSameProductInCarts = (carts: CartItem[], newProduct: Product) => {
